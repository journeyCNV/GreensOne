...
func (c *Container) PrintProviders() []string {
	ret := []string{}
...
// Bind 将服务容器和关键字做了绑定
func (c *Container) Bind(provider ServiceProvider) error {
	c.lock.Lock()
	defer c.lock.Unlock()
	key := provider.Name()
...
func (c *Container) findServiceProvider(key string) ServiceProvider {
	c.lock.RLock()
...
func (c *Container) newInstance(sp ServiceProvider, params []interface{}) (interface{}, error) {
	// force new a
